...
declare -a All_num_layers=(2 4)
declare -a All_hid=(512 1024)

for i in "${!All_env_names[@]}"; do
    for layers in "${All_num_layers[@]}"; do
        for hidden_dim in "${All_hid[@]}"; do
            for seed in "${All_seeds[@]}"; do
                env_name=${All_env_names[$i]}
                task_name=${All_task_names[$i]}
                
                sbatch train_dmc_storm.sh $env_name $task_name $seed 2 $hidden_dim $layers
            done
        done
    done
done
